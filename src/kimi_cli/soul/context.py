...
class Context:
    def __init__(self, file_backend: Path):
        ...
        """Checkpoint IDs present in the active context."""
        self._last_summary: str | None = None
        """The last summary generated by context compaction."""
        ...
        async with aiofiles.open(self._file_backend, encoding="utf-8") as f:
            async for line in f:
                ...
                if line_json["role"] == "_checkpoint":
                    checkpoint_id = line_json["id"]
                    self._checkpoints.add(checkpoint_id)
                    self._next_checkpoint_id = max(self._next_checkpoint_id, checkpoint_id + 1)
                    continue
                if line_json["role"] == "_summary":
                    self._last_summary = line_json["content"]
                    continue
    ...
    async def update_summary(self, summary: str | None):
        """Update and persist the last compaction summary."""
        self._last_summary = summary
        async with aiofiles.open(self._file_backend, "a", encoding="utf-8") as f:
            await f.write(json.dumps({"role": "_summary", "content": summary}) + "\n")
    ...
    async def rewind_to(self, checkpoint_id: int, note: str) -> None:
        ...
        async with (
            ...
            async for line in old_file:
                ...
                if line_json["role"] == "_usage":
                    ...
                    restored_checkpoint_id = line_json["id"]
                    self._checkpoints.add(restored_checkpoint_id)
                    self._next_checkpoint_id = max(
                        self._next_checkpoint_id, restored_checkpoint_id + 1
                    )
                elif line_json["role"] == "_summary":
                    self._last_summary = line_json["content"]
                else:
                    message = Message.model_validate(line_json)
                    self._history.append(message)
