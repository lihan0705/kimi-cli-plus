...
class KimiSoul:
    ...
    async def _checkpoint(self, reason: str = "Turn checkpoint"):
        ...
        # Create a workspace snapshot for this checkpoint
        self._runtime.workspace_checkpoints.create_once(checkpoint_id, reason=reason)
    ...
    async def _turn(self, user_message: Message) -> TurnOutcome:
        ...
        await self._checkpoint(reason=f"User: {text_input[:50]}...")  # this creates the checkpoint 0 on first run
