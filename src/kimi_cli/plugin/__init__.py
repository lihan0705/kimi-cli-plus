        ...
        if isinstance(attr, type) and issubclass(attr, Tool) and attr is not Tool:
            try:
                ...
                if tool_instance:
                    ...
                    logger.info("Loaded Python tool {} from plugin {}", tool_instance.name, plugin.name)
            except Exception as e:
                logger.warning("Failed to instantiate tool {} from plugin {}: {}", attr_name, plugin.name, e)
    ...
    if "__init__" in tool_cls.__dict__:
        ...
        for name, param in sig.parameters.items():
            ...
            if param.annotation in dependencies:
                ...
                logger.debug("Dependency {} not found for tool {}", param.annotation, tool_cls.__name__)
                return None
