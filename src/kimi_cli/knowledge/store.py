...
class KBStore:
    ...
    def upsert_document(self, metadata: DocumentMetadata, content: str) -> None:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute(
                "INSERT INTO content_fts (id, content) VALUES (?, ?)", (data["id"], content)
            )
            conn.commit()
    ...
    def update_document_links(self, doc_id: UUID, content: str) -> None:
        ...
        with self._get_connection() as conn:
            ...
            for target_id_str in resolved_targets:
                conn.execute(
                    ...
                    "(source_id, target_id, link_type) VALUES (?, ?, ?)",
    ...
    def get_related_documents(
        ...
        target_tags: set[str] = set(target.tags)

        # 2. Get all other documents and calculate scores
        related_results: list[tuple[DocumentMetadata, int]] = []
        with self._get_connection() as conn:
            ...
            linked_ids: set[str] = {cast(str, row["linked_id"]) for row in cursor.fetchall()}
            ...
            for row in cursor.fetchall():
                ...
                # Shared tags
                other_tags: set[str] = set(other.tags)
    ...
    def delete_document(self, doc_id: UUID) -> None:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute(
                "DELETE FROM links WHERE source_id = ? OR target_id = ?", (doc_id_str, doc_id_str)
            )
            conn.commit()

    def _row_to_metadata(self, row: sqlite3.Row) -> DocumentMetadata:
        data = dict(row)
        data["tags"] = json.loads(cast(str, data["tags"]))
        data["key_claims"] = json.loads(cast(str, data["key_claims"]))
        data["created_at"] = datetime.fromisoformat(cast(str, data["created_at"]))
        data["updated_at"] = datetime.fromisoformat(cast(str, data["updated_at"]))
    ...
    def get_document(self, doc_id: UUID) -> tuple[DocumentMetadata, str] | None:
        doc_id_str = str(doc_id)
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
            ...
            if not row:
                return None
            return self._row_to_metadata(row), cast(str, row["content"])

    def list_documents(
        self, category: Category | None = None, status: DocumentStatus | None = None
    ...
    def sync_from_disk(self, root: Path) -> None:
        """
        ...
        with self._get_connection() as conn:
            ...
            db_ids: set[str] = {cast(str, row["id"]) for row in cursor.fetchall()}

            ids_to_remove = db_ids - found_ids
            for doc_id in ids_to_remove:
                self.delete_document(UUID(doc_id))

    def _parse_yaml_frontmatter(self, content: str) -> dict[str, object]:
    ...
    def sync_metadata_from_md(self, doc_dir: Path) -> Path:
        ...
        category_value = yaml_data.get("category")
        if isinstance(category_value, str):
            metadata.category = Category(category_value)
        ...
        if _is_str_list(tags):
            metadata.tags = cast(list[str], tags)

        relevance_score = yaml_data.get("relevance_score")
        if isinstance(relevance_score, int):
            metadata.relevance_score = relevance_score

        key_claims = yaml_data.get("key_claims")
        if _is_str_list(key_claims):
            metadata.key_claims = cast(list[str], key_claims)
