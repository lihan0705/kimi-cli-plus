...
from kimi_cli.config import load_config, LLMModel, LLMProvider
...
class Classifier:
    def __init__(self, model: Optional[str] = None, chat_provider: Optional[ChatProvider] = None):
        if chat_provider:
    ...
    async def extract_metadata(self, content: str, category: Category) -> ClassificationResult:
        system_prompt = (
            "You are a knowledge base metadata extractor. Your task is to extract structured metadata "
            ...
            "- temporal_type: 'evergreen' (long-lasting value) or 'time_sensitive' (likely to expire).\n"
