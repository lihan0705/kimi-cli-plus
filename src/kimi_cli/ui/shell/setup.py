...
async def _add_openai_legacy_url() -> bool:
    ...
    name = await _prompt_text(
        "Enter a custom name for this OpenAI Legacy URL (e.g., 'my-openai', 'deepseek')"
    )
    if not name:
        return False
...
async def _list_openai_legacy_urls() -> None:
    ...
    for name, provider in providers:
        ...
        model_count = sum(
            1 for m in config.models.values() if m.provider == make_openai_legacy_provider_key(name)
        )
...
async def _edit_openai_legacy_url() -> bool:
    ...
    name = await _prompt_choice(
        header="Select URL to edit (↑↓ navigate, Enter select, Ctrl+C cancel):", choices=choices
    )
...
async def _delete_openai_legacy_url() -> bool:
    ...
    name = await _prompt_choice(
        header="Select URL to delete (↑↓ navigate, Enter select, Ctrl+C cancel):", choices=choices
    )
    ...
    models_to_delete = [
        key for key, model in config.models.items() if model.provider == provider_key
    ]
    ...
    save_config(config)
    console.print(
        f"[green]✓[/green] Deleted OpenAI Legacy URL '{name}' and {len(models_to_delete)} models"
    )
    return True
