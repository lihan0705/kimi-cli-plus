...
|------------------|-------------------|-------------|
| **📚 LLM Wiki (Filesystem-First)** | • **NEW**: Markdown-first knowledge workflow with no DB dependency on the happy path<br>• `kimi wiki ingest`: Distill URL/file/PDF into canonical wiki pages<br>• `kimi wiki list/read/delete/index`: Basic page lifecycle operations<br>• `kimi wiki relink/audit`: Bidirectional link maintenance with orphan/broken/path/index checks<br>• `KIMI_WIKI_ROOT`: Override wiki root for project-scoped knowledge bases | 2026-04-17 |
