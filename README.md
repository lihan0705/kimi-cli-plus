...
|------------------|-------------------|-------------|
| **🌳 Checkpoint Tree Rewind** | • **NEW**: `/tree` lets you browse session checkpoints and rewind from the terminal shell<br>• Choose **Conversation only** to branch the chat from an earlier checkpoint without touching files<br>• Choose **Conversation + restore files** to restore the workspace snapshot captured before file-writing tools or shell commands changed the project<br>• Restore preview shows files that may change, and Kimi keeps a pre-restore safety snapshot before replacing files | 2026-04-26 |
| **📚 LLM Wiki Commands** | • **NEW**: Markdown-first wiki workflow with filesystem storage on the happy path<br>• CLI: `kimi wiki ingest`, `kimi wiki list`, `kimi wiki read`, `kimi wiki delete`, `kimi wiki index`, `kimi wiki orient`, `kimi wiki import-session`<br>• Shell slash: `/llm-wiki:ingest`, `/llm-wiki:list`, `/llm-wiki:read`, `/llm-wiki:delete`, `/llm-wiki:index`, `/llm-wiki:relink`, `/llm-wiki:audit`, `/llm-wiki:orient`, `/llm-wiki:import-session`<br>• `relink` / `audit`: Bidirectional link maintenance with orphan/broken/path/index checks<br>• `KIMI_WIKI_ROOT`: Override wiki root for project-scoped knowledge bases | 2026-04-17 |
| **🧠 Robust Context Compaction** | • **NEW**: Rolling summaries and intelligent message pruning for long-context stability<br>• **Rolling Summaries**: Incorporates previous summaries to maintain continuity in long conversations<br>• **Smart Pruning**: Automatically prunes oversized tool outputs while preserving key user/assistant exchanges<br>• **Sanitized Pairs**: Ensures tool call/result integrity for better LLM performance | 2026-04-12 |
...
| **🔥 Hot Reload Skills** | • **NEW**: Update skill definitions without restarting Kimi CLI<br>• `/reload-skills`: Instantly refresh all skills and plugins from their roots<br>• Real-time feedback for skill developers and prompt engineering | 2026-03-30 |
