...
def create_doc_dir(
    root: Path, metadata: DocumentMetadata, content: str, yaml_frontmatter: str | None = None
):
...
def test_sync_metadata_basic(store, kb_root):
    ...
    doc_dir = create_doc_dir(
        kb_root,
        meta,
        "Some content",
        yaml_frontmatter="title: New Title\nrelevance_score: 9\ntags: [tag1, tag2]",
    )

    # Pre-sync: upsert to DB
    with open(doc_dir / "document.md") as f:
    ...
    with open(doc_dir / "metadata.json") as f:
...
def test_sync_metadata_category_change(store, kb_root):
    ...
    doc_dir = create_doc_dir(
        kb_root, meta, "Some content", yaml_frontmatter="category: howto\nsubcategory: advanced"
    )
    ...
    with open(new_dir / "metadata.json") as f:
        updated_meta = DocumentMetadata.model_validate_json(f.read())
...
def test_sync_metadata_idempotent(store, kb_root):
    ...
    doc_dir = create_doc_dir(
        kb_root, meta, "Content", yaml_frontmatter="title: Updated\ncategory: howto"
    )
